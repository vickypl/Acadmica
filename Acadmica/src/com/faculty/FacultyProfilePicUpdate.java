...
public class FacultyProfilePicUpdate extends HttpServlet {
...
/*		String projectPath = getServletContext().getRealPath("/");
		...
		try {
			...
			if(fileItems.size()>0) {
				String oldfilepath=fullFilePath+faculty.getRegistrationId()+".jpg";
			...
			for(FileItem item : fileItems) {
				if(!item.isFormField()) {
					...
					if(ext.equals("jpg")) {
						File newFileName = new File(fullFilePath+faculty.getRegistrationId()+"."+ext);
		...
		if(fileName!=null) {
			...
			try {
				...
				String sql ="update facultyprofilepic set url=? where regid=?";
					preStatement = connection.prepareStatement(sql);
					preStatement.setString(1, faculty.getRegistrationId()+".jpg");
					preStatement.setString(2, faculty.getRegistrationId());
		...
		if(status) {
			response.sendRedirect("facultyloggedin.jsp?msg=Profile Picture Updated Succefully..");
		} else {
			response.sendRedirect("facultyloggedin.jsp?msg=Image format should be jpg for dp.");
		}
	}
