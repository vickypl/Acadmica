...
public class AdminProfilePicUpdate extends HttpServlet {
...
/*		String projectPath = getServletContext().getRealPath("/");
		...
		try {
			...
			if(fileItems.size()>0) {
				String oldfilepath=fullFilePath+admin.getRegid()+admin.getFirstName()+".jpg";
			...
			for(FileItem item : fileItems) {
				if(!item.isFormField()) {
					...
					if(ext.equals("jpg")) {
						File newFileName = new File(fullFilePath+admin.getRegid()+admin.getFirstName()+"."+ext);
		...
		if(fileName!=null) {
			...
			try {
				...
				String sql ="update admin set picurl=? where id=?";
					preStatement = connection.prepareStatement(sql);
					preStatement.setString(1, admin.getRegid()+admin.getFirstName()+".jpg");
					preStatement.setString(2, admin.getRegid());
		...
		if(status) {
			response.sendRedirect("deanloggedin.jsp?msg=Profile Picture Updated Succefully..");
		} else {
			response.sendRedirect("deanloggedin.jsp?msg=Image format should be jpg for dp.");
		}
