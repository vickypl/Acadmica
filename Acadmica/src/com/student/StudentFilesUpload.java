...
public class StudentFilesUpload extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//Max file size
		final long MAX_FILE_SIZE = 15000;
				 ...
				 if(fileSize<=MAX_FILE_SIZE && allowed) {
					 for (FileItem fileItem : itemsList) {
							if (!fileItem.isFormField()) {
								...
								try {
									File newFileName = new File(uploadLocation+Calendar.getInstance().getTimeInMillis()+student.getRegistrationId()+"."+ext);
					 ...
					 if(uploadStatus) {
							...
							try {
								...
								String sql ="insert into studentfiles (id, title, filetype, filesize, uploaddate, regid, url) values(stu_file_num.nextval, ?, ?, ?, sysdate, ?, ?)";
									preStatement = connection.prepareStatement(sql);
									preStatement.setString(1, fileTitle);
									preStatement.setString(2, ext);
									preStatement.setString(3, fileSize+"mb");
									preStatement.setString(4, student.getRegistrationId());
						...
						if(updateStatus) {
							response.sendRedirect("studentfiles.jsp?msg=File Successfully Uploaded.");
						}
				 } else {					 
					 response.sendRedirect("studentfiles.jsp?msg=File Size Should be less then 15mb(Supports pdf | docx | txt ).");
				 }
				 
				  
		} else {
			response.sendRedirect("studentfiles.jsp?msg=Something Went Wrong!.");
		}
	}
